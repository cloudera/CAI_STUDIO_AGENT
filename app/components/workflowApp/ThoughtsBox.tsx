  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        >
          {isCollapsed ? (
            <RightOutlined style={{ fontSize: 10, color: 'rgba(0,0,0,0.65)' }} />
          ) : (
            <DownOutlined style={{ fontSize: 10, color: 'rgba(0,0,0,0.65)' }} />
          )}
          <div style={{ fontSize: 11, fontWeight: 600, opacity: 0.85 }}>Thoughts</div>
        </div>
        <span style={{ fontSize: 10, opacity: 0.6 }}>{entries.length}</span>
      </div>
      {!isCollapsed && (
        <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
          ...
          >
            {entries.length === 0 && (
              <div style={{ fontSize: 10, opacity: 0.6 }}>No thoughts yet…</div>
            )}
            {entries.map((entry) => (
              <div key={entry.id} style={{ marginBottom: 6 }}>
                <div style={{ fontSize: 10, lineHeight: 1.4, whiteSpace: 'pre-wrap' }}>
                  {entry.thought}
                </div>
                {entry.tool && (
                  <div style={{ marginTop: 2 }}>
                    <span style={{ fontSize: 10, opacity: 0.6, marginRight: 4 }}>Using Tool</span>
          ...
          <div style={{ flexBasis: '20%', display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ fontSize: 10, opacity: 0.8 }}>Artifacts</div>
