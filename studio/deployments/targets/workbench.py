...
from studio.db.model import DeployedWorkflowInstance, Workflow, Model
...
def get_workbench_llm_config(deployable_workflow_dir: str, artifact: DeploymentArtifact) -> dict:
    if is_custom_model_root_dir_feature_enabled():
        ...
        return {
            ...
            "workflow_artifact_location": os.path.join(
                "/home/cdsw", get_studio_subdirectory(), deployable_workflow_dir, os.path.basename(artifact.project_location)
            ),
...
def prepare_env_vars_for_workbench(
    ...
    workbench_model_config = get_workbench_llm_config(deployable_workflow_dir, artifact)
    
    env_vars_dict.update(
        {
            "AGENT_STUDIO_OPS_ENDPOINT": get_ops_endpoint(),
            "AGENT_STUDIO_WORKFLOW_ARTIFACT_TYPE": "archive",
            "AGENT_STUDIO_WORKFLOW_ARTIFACT": workbench_model_config[
                "workflow_artifact_location"
            ],
...
def deploy_artifact_to_workbench(
    ...
    try:
        ...
        os.makedirs(deployable_workflow_dir, exist_ok=True)
        ...
        deployed_workflow_config = get_workbench_llm_config(deployable_workflow_dir, artifact)
        
        # Deploy workbench model
        cml_model_id, model_build_id = deploy_cml_model(
            cml=cml,
            model_name=cml_model_name,
            model_description=f"Model for workflow {deployment.name}",
        ...
        if payload.deployment_target.deploy_application:
            ...
            if not deployment_metadata.get("application_id"):
                application = create_application_for_deployed_workflow(
                    deployment, False, cml
                )
