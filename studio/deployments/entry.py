...
from studio.deployments.validation import (
...
def deploy_artifact(
    ...
    if payload.deployment_target.type == DeploymentTargetType.WORKBENCH_MODEL:
        ...
        raise ValueError(f'Deploying to a deployment target type of "{payload.deployment_target.type}" is not supported.')
...
def package_workflow_target(payload: DeploymentPayload, deployment: DeployedWorkflowInstance, session: Session, cml: CMLServiceApi) -> DeploymentArtifact:
    """
    ...
    if payload.workflow_target.type == WorkflowTargetType.WORKFLOW:
        ...
        raise ValueError(f'Deployment artifact of type "{payload.workflow_target.type}" is not supported for deployment.')
...
def deploy(payload: DeploymentPayload, session: Session, cml: CMLServiceApi) -> None:
    ...
    try:
        ...
        validate_deployment_payload(payload, session, cml)
...
if __name__ == "__main__":
    ...
    deployment_payload_json: dict = json.loads(args.deployment_payload)
