...
from studio.consts import DEPLOYABLE_WORKFLOWS_LOCATION
...
def package_custom_workflow(
    ...
    if not target.custom_workflow_artifact:
        raise ValueError(
            'Cannot deploy a custom workflow artifact without first specifying "target.custom_workflow_artifact".'
        )
    ...
    return DeploymentArtifact(id=deployment_id, project_location=deployed_workflow_dir)
