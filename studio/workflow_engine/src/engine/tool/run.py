...
def ensure_venv_and_requirements(tool_dir: str, requirements_file: str = "requirements.txt", trace_id=None, tool_instance_id=None):
    ...
    if not uv_bin:
        ...
        if trace_id and tool_instance_id:
            post_tool_event(trace_id, ToolVenvCreationFailedEvent(
    ...
    if trace_id and tool_instance_id:
        post_tool_event(trace_id, ToolVenvCreationStartedEvent(
            timestamp=datetime.utcnow(),
            tool_instance_id=tool_instance_id,
            tool_dir=tool_dir,
            requirements_file=requirements_file,
    ...
    try:
        if os.path.exists(requirements_path):
            ...
            if trace_id and tool_instance_id:
                post_tool_event(trace_id, ToolVenvCreationFinishedEvent(
                    timestamp=datetime.utcnow(),
                    tool_instance_id=tool_instance_id,
                    tool_dir=tool_dir,
                    requirements_file=requirements_file,
...
def run_tool_test(tool_instance_id, tool_dir, user_params, tool_params, trace_id):
    ...
    if missing_args:
        ...
        post_tool_event(trace_id or "unknown", ToolTestFailedEvent(
            timestamp=datetime.utcnow(),
    ...
    post_tool_event(trace_id, ToolTestStartedEvent(
        timestamp=datetime.utcnow(),
        tool_instance_id=tool_instance_id,
    ...
    try:
        ...
        try:
            ...
            post_tool_event(trace_id, ToolTestFailedEvent(
                timestamp=datetime.utcnow(),
                tool_instance_id=tool_instance_id,
                error=f"venv/requirements error: {str(venv_err)}\n{tb}",
            ))
            return {"status": "Tool test failed", "trace_id": trace_id, "error": f"venv/requirements error: {str(venv_err)}\n{tb}"}
        ...
        try:
            ...
            return {"status": "Tool test failed", "trace_id": trace_id, "error": f"subprocess error: {str(sub_err)}\n{tb}\nIf you see ModuleNotFoundError, check if the package is in requirements.txt and that pip install succeeded."}
        output = proc.stdout.strip()
        error = proc.stderr.strip()
        if proc.returncode == 0:
            post_tool_event(trace_id, ToolTestCompletedEvent(
                timestamp=datetime.utcnow(),
                tool_instance_id=tool_instance_id,
                output=output,
            ))
