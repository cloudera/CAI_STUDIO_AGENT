...
# def test_model_test_happy_path(mock_get_llm):
#     # Create a mock LLM object with a call method
#     mock_llm = MagicMock()
#     mock_llm.call.return_value = "Test successful response"
...
#     # Mock environment API key retrieval
...
#         # Verify response
#         assert response.response == "Test successful response"
...
#         # Verify LLM call was made with correct messages
#         mock_llm.call.assert_called_once_with(
#             messages=[{"role": "user", "content": "Test prompt"}]
#         )

# @patch('engine.crewai.llms.get_crewai_llm_object_direct')
# def test_model_test_with_exception(mock_get_llm):
#     # Create a mock LLM object that raises an exception
#     mock_llm = MagicMock()
#     mock_llm.call.side_effect = Exception("Test error")
#     mock_get_llm.return_value = mock_llm
...
#         response = model_test(req, cml=mock_cml, dao=test_dao)
#         assert "Model Test Failed: Test error" in response.response
